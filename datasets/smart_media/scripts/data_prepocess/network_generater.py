...
class NetworkGenerater:
    ...
    def get_max_len(self, field_name):
        if field_name in self.multiple_fields:
            return 50
        else:
            return 1
    ...
    def print_train_slots(self, fout, field_num, field_name_list):
        fout.write('slots = {\n')
        ...
        fout.write('\t\"max_seq_len\": [%s, 1],\n' %(
            self.tidy(', '.join([str(self.get_max_len(field_name)) for field_name \
                    in field_name_list]))))
    ...
    def generate_data_feed(self, features_map_file, data_feed_file):
        self.__init__()
        ...
        self.print_train_slots(data_f, field_num, field_name_list)
